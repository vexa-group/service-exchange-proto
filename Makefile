...
py-deps:
	...
	@mkdir -p gen/python/service_exchange_proto
	@touch gen/python/service_exchange_proto/__init__.py
	@poetry env use 3.12
	@poetry install
...
go:
	...
	@protoc \
		--proto_path=proto \
		--go_out=. --go_opt=module=github.com/vexa-group/service-exchange-proto \
		--go-grpc_out=. --go-grpc_opt=module=github.com/vexa-group/service-exchange-proto \
		proto/**/*.proto
...
py:
	...
	@poetry run python -m grpc_tools.protoc \
		--proto_path=proto \
		--python_out=gen/python/service_exchange_proto \
		--grpc_python_out=gen/python/service_exchange_proto \
		proto/**/*.proto
