...
This script:
1. Moves proto files from exchange/ to their respective service folders
...
SERVICES = ["coin", "common", "exchange", "market", "order", "provider", "trade"]


def get_base_dir() -> Path:
    """Get the base directory for generated Python code."""
    return Path("gen/python/service_exchange_proto")
...
def fix_imports_in_file(file_path: Path, base_dir: Path) -> None:
    ...
    # Pattern: from exchange import auth_pb2 as ... -> from . import auth_pb2 as ...
    content = re.sub(
        r"from exchange import " + service + r"_pb2 as (\S+)",
        r"from . import " + service + r"_pb2 as \1",
        content,
    )

    # Fix cross-service imports (absolute imports)
    # Pattern: from exchange import common_pb2 as ... -> from service_exchange_proto.common import common_pb2 as ...
    for other_service in SERVICES:
        if other_service == service:
            continue
        # Match: from exchange import {other_service}_pb2 as ...
        content = re.sub(
            r"from exchange import " + other_service + r"_pb2 as (\S+)",
            r"from service_exchange_proto."
            + other_service
            + r" import "
            + other_service
            + r"_pb2 as \1",
...
def update_top_level_init(base_dir: Path) -> None:
    ...
    content = """\"\"\"Service Exchange Proto - Python gRPC bindings.

This package contains the generated Python code for the Exchange Service gRPC API.
\"\"\"

"""
    for service in SERVICES:
        content += f"from service_exchange_proto import {service}\n"
    content += "\n__all__ = [\n"
    for service in SERVICES:
        content += f"    '{service}',\n"
    content += "]\n"
...
def cleanup_exchange_directory(base_dir: Path) -> None:
    """Remove the exchange directory if it's empty."""
    ...
    if exchange_dir.exists() and exchange_dir.is_dir():
        ...
        files = list(exchange_dir.glob("*"))
        files = [f for f in files if f.name != "__init__.py"]
        if not files:
            shutil.rmtree(exchange_dir)
            print(f"Removed empty directory: {exchange_dir}")
        else:
            print(
                f"Warning: exchange directory not empty, contains: {[f.name for f in files]}"
            )
...
def main() -> None:
    ...
    exchange_dir = get_service_dir(base_dir)
    ...
    if not exchange_dir.exists():
        print(f"Error: Exchange directory {exchange_dir} does not exist!")
        sys.exit(1)
    ...
    moved_files = move_proto_files(base_dir, exchange_dir)
    ...
    # Step 6: Cleanup
    print("\n=== Step 6: Cleanup ===")
    cleanup_exchange_directory(base_dir)
